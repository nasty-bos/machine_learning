...
# Load the datasets
trainSet = pd.read_hdf(os.path.join(cd.data_dir(), cd.DataSets.EX3.value, 'train.h5'), key='train')
testSet = pd.read_hdf(os.path.join(cd.data_dir(), cd.DataSets.EX3.value, 'test.h5'), key='test')
sampleSet = pd.read_csv(os.path.join(cd.data_dir(), cd.DataSets.EX3.value, 'sample.csv'), header=0, index_col=0, float_precision='round_trip')
...
xColumns = ['x' + str(i+1) for i in range(100)]
yColumns = ['y']
...
yPred.to_csv(os.path.join(cd.data_dir(), cd.DataSets.EX3.value, 'sample.csv'))
